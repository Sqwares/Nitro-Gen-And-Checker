...
class NitroGen: # Initialise the class
    ...
    def main(self): # The main function contains the most important code
        ...
        if os.name == "nt": # If the system is windows
            print("")
            ctypes.windll.kernel32.SetConsoleTitleW("Nitro Generator and Checker - Made by Sqwares") # Change the
        else: # Or if it is unix
            print(f'\33]0;Nitro Generator and Checker - Made by Sqwares\a', end='', flush=True) # Update title of command prompt
...
|  $$$$$$  /$$__  $$| $$ | $$ | $$ |____  $$ /$$__  $$ /$$__  $$ /$$_____/
 ...
 /$$  \ $$| $$  | $$| $$ | $$ | $$ /$$__  $$| $$      | $$_____/ \____  $$
|  $$$$$$/|  $$$$$$$|  $$$$$/$$$$/|  $$$$$$$| $$      |  $$$$$$$ /$$$$$$$/
 \______/  \____  $$ \_____/\___/  \_______/|__/       \_______/|_______/ 
                | $$                                                      
                | $$                                                      
                |__/                                                      
                                                        """) # Print the title card
        time.sleep(2) # Wait a few seconds
        self.slowType("Made by: Sqwares#4767 tiktok sqwares_", .02) # Print who developed the code
        time.sleep(1) # Wait a little more
        self.slowType("\nI: ", .02, newLine = False) # Print the first question

        num = int(input('')) # Ask the user for the amount of codes

        # Get the webhook url, if the user does not wish to use a webhook the message will be an empty string
        self.slowType("\nDiscord Webhook kullanmak istermisin eğer kullanmak istemiyorsan entere bas: ", .02, newLine = False)
        ...
        for i in range(num): # Loop over the amount of codes to check
            ...
            if os.name == "nt": # If the system is windows
                ctypes.windll.kernel32.SetConsoleTitleW(f"Nitro Generator and Checker - {len(valid)} Valid | {invalid} Invalid - Made by Sqwares#4767") # Change the title
                print("")
            else: # If it is a unix system
                print(f'\33]0;Nitro Generator and Checker - {len(valid)} Valid | {invalid} Invalid - Made by Sqwares#4767\a', end='', flush=True) # Change the title
...
Results:
 ...
 Valid Codes: {', '.join(valid )}""") # Give a report of the results of the check
        ...
        input("\nTarama Sonlandı Programı Kapatmak İçin 5 Kere entere basın.") # Tell the user the program finished
                ...
                if response.status_code == 200: # If the responce went through
                    ...
                    if notify is not None: # If a webhook has been added
                        DiscordWebhook( # Send the message to discord letting the user know there has been a valid nitro code
                            url = notify,
                            content = f"Geçerli Nitro Bulundu! @everyone \n{nitro}"
                        ).execute()
